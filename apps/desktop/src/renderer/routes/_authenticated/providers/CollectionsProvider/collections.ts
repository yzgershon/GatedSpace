...
const createPersistedElectricCollection = ((config: ElectricSyncConfig) => {
	// Local-only mode has no Electric backend: every cloud-synced collection
	// becomes an in-memory local-only store, seeded on boot by
	// LocalModeSeeder (org/user/host rows + host project.list mirror).
	if (isLocalMode()) {
		return createCollection({
			...localOnlyCollectionOptions({
				id: config.id,
				// biome-ignore lint/suspicious/noExplicitAny: electric config generics widen
